...
import '../interfaces/callback/IPegasysV2MintCallback.sol';
import '../interfaces/callback/IPegasysV2SwapCallback.sol';
import '../interfaces/callback/IPegasysV2FlashCallback.sol';

import '../interfaces/IPegasysV2Pool.sol';

contract TestPegasysV2Callee is IPegasysV2MintCallback, IPegasysV2SwapCallback, IPegasysV2FlashCallback {
    ...
    function swapToLowerSqrtPrice(address pool, uint160 sqrtPriceX96, address recipient) external {
        IPegasysV2Pool(pool).swap(recipient, true, type(int256).max, sqrtPriceX96, abi.encode(msg.sender));
    }

    function swapToHigherSqrtPrice(address pool, uint160 sqrtPriceX96, address recipient) external {
        IPegasysV2Pool(pool).swap(recipient, false, type(int256).max, sqrtPriceX96, abi.encode(msg.sender));
    }
    ...
    function pegasysV2SwapCallback(int256 amount0Delta, int256 amount1Delta, bytes calldata data) external override {
        ...
        if (amount0Delta > 0) {
            IERC20Minimal(IPegasysV2Pool(msg.sender).token0()).transferFrom(sender, msg.sender, uint256(amount0Delta));
        } else if (amount1Delta > 0) {
            IERC20Minimal(IPegasysV2Pool(msg.sender).token1()).transferFrom(sender, msg.sender, uint256(amount1Delta));
        } else {
    ...
    function mint(address pool, address recipient, int24 tickLower, int24 tickUpper, uint128 amount) external {
        IPegasysV2Pool(pool).mint(recipient, tickLower, tickUpper, amount, abi.encode(msg.sender));
    }

    event MintCallback(uint256 amount0Owed, uint256 amount1Owed);

    function pegasysV2MintCallback(uint256 amount0Owed, uint256 amount1Owed, bytes calldata data) external override {
        address sender = abi.decode(data, (address));

        emit MintCallback(amount0Owed, amount1Owed);
        if (amount0Owed > 0)
            IERC20Minimal(IPegasysV2Pool(msg.sender).token0()).transferFrom(sender, msg.sender, amount0Owed);
        if (amount1Owed > 0)
            IERC20Minimal(IPegasysV2Pool(msg.sender).token1()).transferFrom(sender, msg.sender, amount1Owed);
    }
    ...
    function flash(
        ...
        IPegasysV2Pool(pool).flash(recipient, amount0, amount1, abi.encode(msg.sender, pay0, pay1));
    }

    function pegasysV2FlashCallback(uint256 fee0, uint256 fee1, bytes calldata data) external override {
        emit FlashCallback(fee0, fee1);

        (address sender, uint256 pay0, uint256 pay1) = abi.decode(data, (address, uint256, uint256));

        if (pay0 > 0) IERC20Minimal(IPegasysV2Pool(msg.sender).token0()).transferFrom(sender, msg.sender, pay0);
        if (pay1 > 0) IERC20Minimal(IPegasysV2Pool(msg.sender).token1()).transferFrom(sender, msg.sender, pay1);
    }
}
