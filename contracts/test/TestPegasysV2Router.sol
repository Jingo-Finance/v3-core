...
contract TestPegasysV2Router is IPegasysV2SwapCallback {
    ...
    function swapForExact0Multi(
        ...
        pools[0] = poolInput;
        IPegasysV2Pool(poolOutput).swap(
            recipient,
            false,
            -amount0Out.toInt256(),
    ...
    function pegasysV2SwapCallback(
        ...
        if (pools.length == 1) {
            ...
            address tokenToBePaid =
                amount0Delta > 0 ? IPegasysV2Pool(msg.sender).token0() : IPegasysV2Pool(msg.sender).token1();
            int256 amountToBePaid = amount0Delta > 0 ? amount0Delta : amount1Delta;

            bool zeroForOne = tokenToBePaid == IPegasysV2Pool(pools[0]).token1();
            IPegasysV2Pool(pools[0]).swap(
                msg.sender,
                zeroForOne,
                -amountToBePaid,
            ...
            if (amount0Delta > 0) {
                IERC20Minimal(IPegasysV2Pool(msg.sender).token0()).transferFrom(
                    payer,
                    msg.sender,
                    uint256(amount0Delta)
                );
            } else {
                IERC20Minimal(IPegasysV2Pool(msg.sender).token1()).transferFrom(
                    payer,
                    msg.sender,
