...
import { TestPegasysV2SwapPay } from '../typechain/TestPegasysV2SwapPay'
...
import { TestPegasysV2ReentrantCallee } from '../typechain/TestPegasysV2ReentrantCallee'
...
describe('PegasysV2Pool', () => {
  ...
  let factory: PegasysV2Factory
  let pool: MockTimePegasysV2Pool

  let swapTarget: TestPegasysV2Callee
  ...
  async function initializeAtZeroTick(pool: MockTimePegasysV2Pool): Promise<void> {
    await pool.initialize(encodePriceSqrt(1, 1))
  ...
  describe('#lock', () => {
    ...
    it('cannot reenter from swap callback', async () => {
      const reentrant = (await (
        await ethers.getContractFactory('TestPegasysV2ReentrantCallee')
      ).deploy()) as TestPegasysV2ReentrantCallee
  ...
  describe('swap underpayment tests', () => {
    let underpay: TestPegasysV2SwapPay
    beforeEach('deploy swap test', async () => {
      const underpayFactory = await ethers.getContractFactory('TestPegasysV2SwapPay')
      underpay = (await underpayFactory.deploy()) as TestPegasysV2SwapPay
